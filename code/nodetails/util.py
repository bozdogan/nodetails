...
def prepare_for_training(data: pd.DataFrame, max_len_text, max_len_sum):
    if _DEBUG:
...
def prepare_dataset(datafile, max_len_text, max_len_sum, nrows=None, show_histogram=False):
    data = preprocess_dataset(datafile, nrows, show_histogram)
    return prepare_for_training(data, max_len_text, max_len_sum)
